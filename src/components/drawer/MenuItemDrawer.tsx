...
const MenuItemDrawer: FC<MenuItemPopupProps> = ({
  children,
  item,
  setChoose,
}) => {
  ...
  return (
    <Drawer open={open} onOpenChange={setOpen}>
      ...
      <DrawerContent
        className={cn(
          "flex w-full flex-col justify-end border-[1px] border-menubackground bg-menubackground md:hidden lg:w-0",
          !item.modifiers ? "h-[50%]" : "h-[90%]",
        )}
      >
        ...
        <div
          style={{
            backgroundImage:
              item.images[0] && !item?.extras?.hideMenuThumbNailImages
                ? `url(${item.images[0]})`
          ...
          className={cn(
            ...
            (item.extras?.hideMenuThumbNailImages || !item.images[0]) && "h-0",
          )}
        ></div>
        <Button
        ...
        >
              ...
              {item && (
                <div className="flex flex-col gap-2">
                  <div className="z-40 flex w-full justify-between gap-2">
                    <p className="w-[70%] text-lg font-semibold text-menusecondary">
                      {item.name}
                    </p>
          ...
          {item.modifiers.map((mod, index) => {
            return (
              <div
                className="rounded-xl bg-menuforeground px-5 py-6"
                key={index}
              >
                <div className="flex flex-col gap-4 space-y-2">
                  ...
                  <div className="z-40 flex h-full w-full flex-col gap-6">
                    {mod?.multiSelection
                              ...
                              <div className="flex w-fit items-center justify-start gap-5">
                                <div className="flex flex-col items-start justify-center">
                                  <Label
                                    ...
                                    className="items-center gap-2 text-menusecondary"
                                  >
                                    {modifier.name}
                                  </Label>
                                  <Label
                      ...
                      : item &&
                        GetModifiersFromItemId(item, items, index).length >
                          0 && (
                          <div>
                            <RadioGroup
          ...
          (restaurant?.isDeliveryEnabled || restaurant.isTakeAwayEnabled) && (
            <DrawerFooter className="flex w-full flex-row justify-start gap-5">
              <div className="flex h-12 w-1/2 items-center justify-center gap-3 rounded-none bg-menuprimary p-2 text-menuforeground">
                ...
                <span className="text-lg font-medium text-menuforeground">
                  {quantity}
                </span>
                <Button
