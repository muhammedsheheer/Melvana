...
const TimeForm: FC<TimeFormProps> = ({ mainform, setpage }) => {
  ...
  return (
    <div className="flex w-full flex-col items-center justify-center gap-3">
      <p className="font-marcellus text-3xl font-semibold text-[#fff]">
      ...
      <Form {...form}>
        ...
        >
          <div className="grid w-full max-w-[700px] grid-cols-1 gap-6 py-12">
            <FormField
                  ...
                  <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
                    <PopoverTrigger asChild>
                      <FormControl>
                        <Button
                          variant={"outline"}
                          className={`h-12 w-full justify-start border-muted bg-transparent text-left font-normal text-[#fff] hover:bg-[#000000] hover:text-[#fff]`}
                        >
              ...
              <div className="custom-scrollbar grid h-[250px] grid-cols-2 gap-6 overflow-y-scroll md:grid-cols-4 lg:grid-cols-5">
                {generateTimeSlots().map((time) => (
                  <div
                    ...
                    className={cn(
                      "flex h-full w-full cursor-pointer items-center justify-center border-[1px] border-muted py-4 text-[#fff]",
                      form.watch("time") === time && "border-[#A98151]",
                    )}
                  >
                    {time}
                  </div>
